...
/**
 * Zeilen für Auftragnehmer auf Verträgen. Fallback: Systemhaus-Ess.
 */
export function contractorPartyLines(org?: OrgAddressFields | null): string[] {
  const customName = org?.orgName?.trim() || "";
  const name = customName || "Systemhaus-Ess";
  const tagline = org?.orgTagline?.trim() || (customName ? "" : "IT-Dienstleistungen & Support");
