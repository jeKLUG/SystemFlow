...
Nav „Monitoring“ (`/monitoring`): Flotte, Warnungskarten, Agent-Zuordnung, Kundenkacheln. Kundenseite (`/monitoring/customers/:id`): Diagramme, Meldungen, durchsuchbare Geräteliste und Gerätedetail. Agent einrichten (`/monitoring/setup`): Enrollment-Key, Pakete, Install-Skripte.
...
Staff-Nav **Monitoring** (`/monitoring`): Flotten-Dashboard, Warnungen als Karten, Zuordnung unzugeordneter Agenten zum Inventar, Einstieg je Kunde. Kundenseite (`/monitoring/customers/:customerId`): Status-Diagramme, Meldungen, durchsuchbare Geräteliste; Gerätedetail mit Verlauf (30 Tage) unter `/monitoring/customers/:customerId/devices/:assetId`. Kundenportal hat in V1 keine Monitoring-Ansicht.
