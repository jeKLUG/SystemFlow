...
- **org_settings** – Standard-Stundensatz, Währung, MwSt.-Hinweis (UI: Preise-App); Firmenanschrift (UI: Konto) als Auftragnehmer auf Verträgen/PDF; `monitoringEnrollmentKey` für Agent-Anmeldung; `marketingSignatureHtml` für die Akquise-Signatur
...
- **monitoring_agents** / **monitoring_samples** / **monitoring_agent_packages** / **monitoring_script_templates** / **monitoring_script_jobs** – Live-Agenten (Windows/Linux) mit Heartbeat, Inventar-Zuordnung, 30-Tage-Verlauf; aktuelles Binary je Plattform (`windows-amd64`, `linux-amd64`, `linux-arm64`) in den Uploads; org-weite PowerShell-Vorlagen und Aufträge (Ausgabe, Exitcode)
...
- **marketing_lists** / **marketing_leads** / **marketing_templates** / **marketing_sends** – Akquise (Staff `/marketing`): eigene Leads getrennt von `customers`, Textbausteine Erstmail/Erinnerung, Versandprotokoll. Abmelde-Token am Lead (`unsubToken`). Listen mit erfolgreichem Versand werden archiviert (`archivedAt`) und dürfen nicht gelöscht werden.
...
Nav „Marketing“ (`/marketing`, Werkzeuge): Reiter Empfänger, Archiv, Texte, Versand. Listen und Leads (E-Mail, Firma, optionaler Ansprechpartner), Textbausteine mit Platzhaltern `{{firma}}` / `{{ansprechpartner}}`, globale HTML-Signatur unter dem Text, Versand über dasselbe SMTP wie die Transaktionsmails. Status je Lead (Neu / Gesendet / Erinnerung fällig nach 7 Tagen / Erinnert / Geantwortet / Kontakt / Abgemeldet). Erinnerung nur mit eigenem Baustein. „Geantwortet“ ist manuell plus Notiz; daraus kann ein Kontakt (`customers.kind = contact`) entstehen. Bestehende Kunden-E-Mails werden übersprungen. Öffentliche Abmeldung unter `/m/unsubscribe/:token` (ohne Login). Kein Portal, kein IMAP. Nach erfolgreichem Versand liegt die Liste im Reiter Archiv (nicht löschbar, Versanddatum sichtbar); Erinnerungen gehen weiterhin.
...
- **Inventar:** Am Asset `monitoringEnabled` (Zuordnungsziel), je Warnungstyp aktiv/Priorität (`monitoringAlertsJson`), optionale ICMP-Ziele (`monitoringPingTargetsJson`) und Dienst-Wächter (`monitoringServiceWatchesJson`). Konfiguration im Gerätedetail: Pings und Dienste direkt in der Geräteansicht, Ticket-Schalter hinter „Ticket-Typen“. Datenträger zusätzlich: Standard-Belegt-% und je Laufwerk eigener Schwellwert (`disk.volumes`). Ohne aktivierte Typen bleibt ein PC nach Feierabend still – nur Anzeige online/offline.
- **Agent** (`apps/agent`, Go, ab 1.0.11): Windows-Dienst `SystemhausAgent` (Start automatisch, Neustart nach Absturz) bzw. Linux-systemd `systemhaus-agent` (`Restart=always`). Meldet Laufwerke, **verbaute Hardware** (System/Mainboard/BIOS, CPU, RAM-Riegel, physische Datenträger inkl. Health/SMART, GPU, NICs; Windows per WMI, alle 6 h), Sitzung, Gateway/DNS/DHCP inkl. Erreichbarkeit, öffentliche IP, fehlgeschlagene Auto-Start-Dienste, installierte Software (alle 6 h), Defender/Firewall, unerwartete Neustarts, **konfigurierte Pings** (ICMP, max. 8 Ziele), **Dienst-Wächter** (max. 8 Dienste aus dem Heartbeat), **Uhrzeit** (NTP, Fallback HTTP Date) und **Prozesse** (CPU plus RAM, bis 60). Windows ab 1.0.11 führt **PowerShell-Aufträge** aus dem Gerätedetail als LocalSystem aus (ein Auftrag, Timeout 60 s, stdout/stderr im nächsten Heartbeat). Im Gerätedetail Überblickskarten – Klick auf CPU oder RAM öffnet die Prozessliste – Schutz (Antivirus/Firewall), Netzpanel mit LAN/WAN/Gateway/DNS/Uhrzeit und eigenen Ping-Zielen, Datenträger, Dienste (Wächter plus Auto-Start-Fehler), Softwareliste und PowerShell. Push jede Minute an `POST /api/monitoring/heartbeat`. Enrollment mit Schlüssel von `/monitoring/setup` (`POST /api/monitoring/enroll`). Gleiche Machine-ID (`MachineGuid` `/etc/machine-id`) erzeugt keinen zweiten Pending-Eintrag. Staff kann unzugeordnet oder am Gerät **Client löschen**: Heartbeat-Flag `uninstall`, Agent deinstalliert den Dienst inkl. Binary/Config und verschwindet aus der Flotte.
...
- **Schwellwerte:** CPU/RAM > 90 % über 5 Minuten; Event-Log/Journal-Fehler; ausstehende Updates; Datenträger-Gesundheit (SMART/HealthStatus warn/fail); fehlgeschlagene Auto-Start-Dienste; Neustart ausstehend; Antivirus (Echtzeitschutz aus oder Signaturen > 7 Tage); aktives Firewall-Profil aus; unerwarteter Neustart (Event 6008/41/1001); Gateway-Ping oder DNS-Auflösung fehlgeschlagen; konfigurierte ICMP-Pings ohne Antwort; überwachte Dienste nicht laufend; Geräteuhr weicht ≥ 30 s von NTP ab. Datenträger voll: Standard 90 % belegt, je Laufwerk am Gerät überschreibbar; ein Ticket pro Laufwerk über dem Wert. Ping: ein Ticket je Ziel (`ping:<id>`). Dienst-Wächter: ein Ticket je Dienst (`svc:<id>`). Übrige Typen: ein Ticket pro Typ (Heartbeat und Offline-Prüfung nicht parallel). Auto-Close wenn ok oder deaktiviert. Quelle `monitoring`. Geräte mit gespeichertem `monitoringAlertsJson` haben die neuen Typen zunächst aus – unter „Ticket-Typen“ einschalten.
