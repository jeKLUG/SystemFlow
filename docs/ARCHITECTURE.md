...
Nav „Preise“ (`/prices`): Preiskatalog (Stunde/Pauschale/Stück) und Standardpreise; Konto (`/settings`) enthält Passwort, E-Mail (SMTP/Typen als kompakte Zeilen, Bearbeitung im Dialog) und Sicherung.
...
Staff-Nav **Monitoring** (`/monitoring`): Flotten-Dashboard, Warnungen als Zeilen, Zuordnung unzugeordneter Agenten zum Inventar, Kunden als Zeilen mit Geräte-/Warnungszahlen. Kundenseite (`/monitoring/customers/:customerId`): Status-Diagramme, Meldungen, durchsuchbare Geräteliste; Gerätedetail mit Verlauf (30 Tage) unter `/monitoring/customers/:customerId/devices/:assetId`. Kundenportal hat keine Monitoring-Diagramme; im Inventar steht, ob ein Gerät überwacht wird.
...
- **Schwellwerte:** CPU/RAM > 90 % über 5 Minuten; Event-Log/Journal-Fehler; ausstehende Updates. Datenträger: Standard 90 % belegt, je Laufwerk am Gerät überschreibbar; ein Ticket pro Laufwerk über dem Wert. Übrige Typen: ein Ticket pro Typ (Heartbeat und Offline-Prüfung nicht parallel). Auto-Close wenn ok oder deaktiviert. Quelle `monitoring`.
