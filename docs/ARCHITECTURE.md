...
- **Schwellwerte:** CPU/RAM > 90 % über 5 Minuten; Event-Log/Journal-Fehler; ausstehende Updates; Datenträger-Gesundheit (SMART/HealthStatus warn/fail); fehlgeschlagene Auto-Start-Dienste; Neustart ausstehend; Antivirus (Echtzeitschutz aus oder Signaturen > 7 Tage); aktives Firewall-Profil aus; unerwarteter Neustart (Event 6008/41/1001); Gateway-Ping oder DNS-Auflösung fehlgeschlagen; konfigurierte ICMP-Pings ohne Antwort. Datenträger voll: Standard 90 % belegt, je Laufwerk am Gerät überschreibbar; ein Ticket pro Laufwerk über dem Wert. Ping: ein Ticket je Ziel (`ping:<id>`). Übrige Typen: ein Ticket pro Typ (Heartbeat und Offline-Prüfung nicht parallel). Auto-Close wenn ok oder deaktiviert. Quelle `monitoring`. Geräte mit gespeichertem `monitoringAlertsJson` haben die neuen Typen zunächst aus – unter „Ticket-Typen“ einschalten.
