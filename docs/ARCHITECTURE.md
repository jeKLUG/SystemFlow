...
Staff-Nav **Monitoring** (`/monitoring`): Flotten-Dashboard, Warnungen als Zeilen, Zuordnung unzugeordneter Agenten zum Inventar, Kunden als Zeilen mit Geräte-/Warnungszahlen. Kundenseite (`/monitoring/customers/:customerId`): Status-Diagramme, Meldungen, durchsuchbare Geräteliste; Gerätedetail mit Verlauf (30 Tage) unter `/monitoring/customers/:customerId/devices/:assetId`. Kundenportal hat in V1 keine Monitoring-Ansicht.
