...
UI: `/marketing` (Navbar Werkzeuge). Eigene Leads, nicht die Kontakte-Tabelle. Versand über SMTP aus `/api/settings/mail`.
...
| POST | `/api/monitoring/heartbeat` | Header `Authorization: Bearer <token>`. Body: CPU/RAM/`disks[]`/`hardware`/`session`/`network`/`services[]`/`software[]`/`defender`/`firewall`/`crash`/`pings[]`/`watchedServices[]` (`id`,`name`,`ok`,`state?`)/`clock` (`offsetSec`,`source`,`ok`)/`processes[]`/`updates.rebootPending`/`platform` plus optional `scriptResult` (`jobId`, `exitCode?`, `stdout?`, `stderr?`, `timedOut?`, `error?`). Antwort: `{ ok, assigned, updateNow, uninstall, pingTargets: [{ id, host }], serviceWatches: [{ id, name }], latestAgent?: { platform, version, sha256 }, scriptJob?: { id, timeoutSec, script } }` – `scriptJob` nur Windows-Agent ≥ 1.0.11 |
...
| GET | `/api/monitoring/devices/:assetId?from=&to=` | Snapshot + Samples (`from`/`to` Unix-ms); `software[].match` gegen Kunden-Inventar `software`/`license`; `scriptJobs[]` (letzte 20), `scriptTemplates[]`, `scriptCapable`, `scriptWindows` |
...
| POST | `/api/monitoring/devices/:assetId/scripts` | PowerShell-Auftrag `{ script, templateId?, timeoutSec? }` (max. 32 KB, ein offener Job, Windows-Agent ≥ 1.0.11). Ausführung als LocalSystem beim nächsten Heartbeat |
| GET | `/api/monitoring/script-templates` | Org-weite PowerShell-Vorlagen |
| POST | `/api/monitoring/script-templates` | `{ name, body }` |
| PATCH | `/api/monitoring/script-templates/:id` | `{ name?, body? }` |
| DELETE | `/api/monitoring/script-templates/:id` | Vorlage löschen |
